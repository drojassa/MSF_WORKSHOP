...
public:
  ...
  double C_vel(int ix,int iy,bool usenew);
  double f_eq(int ix,int iy,double rho0,double Jx0,double Jy0,int i); //función de equilibrio
...
/*la función que hace el cambio de indice de refracción de forma suave:
 como n_i=C/v_i con v_i la velocidad del medio, tenemos que pasar de 0.5 a 0.25
 para que los indices cambien de 1 a 2.*/
double LatticeBoltzmann::Ccelda(int ix,int iy){
  int ix0=100;    //posición en x del cambio de índice
  double c1=0.5;  //vel del medio 1
  double c2=0.25; //vel del medio 2

  return ((c2-c1)/2)*std::tanh(ix-ix0)+((c2+c1)/2);
}
...
double LatticeBoltzmann::f_eq(int ix, int iy,double rho0,double Jx0,double Jy0,int i){
  /*C<0.707 cells/click velocidad de las ondas, c no puede ser mayor
    que 1 ya que permitiría información de segundos vecinos e
...
void LatticeBoltzmann::Start(double rho0,double Jx0,double Jy0){
  ...
  for(ix=0;ix<Lx;ix++){  //para cada celda ix,iy
	...
	n0=n(ix,iy,i);
	f[n0]=f_eq(ix,iy,rho0,Jx0,Jy0,i); //se rellena inicialmente cada celda con
      }                                   //los valores de f_eq según los valores 
    }                                     //iniciales de rho y J, y la casilla
  }                                       //para revisar la velocidad.
}
...
void LatticeBoltzmann::Collision(void){
  ...
  for(ix=0;ix<Lx;ix++){
    for(iy=0;iy<Ly;iy++){
      ...
      for(i=0;i<Q;i++){
	n0=n(ix,iy,i);
	fnew[n0]=UmUtau*f[n0]+Utau*f_eq(ix,iy,rho0,Jx0,Jy0,i);
      }
    }
  }
}
...
void LatticeBoltzmann::ImposeField(int t){
  ...
  double Cf=0.5; // velocidad del forzamiento
  double omega=2*M_PI/lambda*Cf; //vel angular del forzamiento
  
  //fuente oscilante en la pared con ix=0
  ix=0;
  rho0=10*sin(omega*t);
  for(iy=0;iy<Ly;iy++){
    ...
    for(i=0;i<Q;i++){
      n0=n(ix,iy,i);
      fnew[n0]=f_eq(ix,iy,rho0,Jx0,Jy0,i);
    }
  }
}
