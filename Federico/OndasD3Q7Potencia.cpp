...
void LatticeBoltzmann::Collision(void){
  ...
  //Salón grande es de 26.6m a 21.05m 

  //Pared derecha (ix=Lx-1)
  for(i=1;i<Q;i++){
    //Pared más grande
    for(iy=0,ix=Lx-1;iy<floor(0.79*Ly);iy++){
      for(iz=0;iz<floor(0.51*Lz)-1;iz++){
        n0=n(ix,iy,iz,i);
        n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
        fnew[n0f]=D*f[n0];        
        }
      }
    for(iy=0,ix=Lx-1;iy<floor(0.78*Ly);iy++){
      for(iz=floor(0.51*Lz)-1;iz<Lz;iz++){
        n0=n(ix,iy,iz,i);
        n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
        fnew[n0f]=D*f[n0]; 
        }
      }
    //Paredes más chikis >w<
    for(iz=0;iz<floor(0.51*Lz);iz++){
      for(ix=0,iy=floor(0.79*Ly)-1;iy<floor(0.97*Ly);iy++){
        n0=n(ix,iy,iz,i);
        n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
        fnew[n0f]=D*f[n0]; 
        }
      for(ix=floor(0.19*Lx)-1;ix<floor(0.19*Lx)+1;ix++){
        for(iy<floor(0.79*Ly)-1;iy<(0.97*Ly);iy++){
          n0=n(ix,iy,iz,i);
          n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
          fnew[n0f]=D_Drywall*f[n0]; 
          }
        }
      for(ix=floor(0.35*Lx)-1;ix<floor(0.35*Lx)+1;ix++){
        for(iy=floor(0.79*Ly)-1;iy<floor(0.97*Ly);iy++){
          n0=n(ix,iy,iz,i);
          n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
          fnew[n0f]=D*f[n0]; 
          }
        }
      for(ix=floor(0.5*Lx)-1;ix<floor(0.5*Lx)+1;ix++){
        for(iy=floor(0.79*Ly)-1;iy<Ly;iy++){
          n0=n(ix,iy,iz,i);
          n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
          fnew[n0f]=D_Drywall*f[n0]; 
          }
        }
      for(ix=floor(0.84*Lx)-1;ix<floor(0.84*Lx)+1;ix++){
        for(iy=floor(0.79*Ly)-1;iy<Ly;iy++){
          n0=n(ix,iy,iz,i);
          n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
          fnew[n0f]=D_Drywall*f[n0];          
  ...
  //fronteras del frente
  for(i=1;i<Q;i++){
    //Para la parte inferior 2.1m
    for(iz=0;iz<floor(Lz*0.51)-1;iz++){ //Para la parte inferior 2.1m
      for(ix=0;ix<floor(0.36*Lx)-1;ix++){ //Primer salón y segundo salón
      //de 0m a 9.3m
        for(iy=floor(0.98*Ly)-1;iy<Ly;iy++){
          //de 20.5m a 21.05m
          n0=n(ix,iy,iz,i);
          n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
          fnew[n0f]=D*f[n0];  
      ...
      for(iy=Ly-1,ix=floor(0.36*Lx)-1;ix<floor(0.57*Lx)-1;ix++){//Entrada del comedor
        //de 9.3m a 13.3m 
        n0=n(ix,iy,iz,i);
        n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
        fnew[n0f]=0*f[n0]; 
        }
      for(iy=Ly-1,ix=floor(0.57*Lx)-1;ix<floor(0.84*Lx)-1;ix++){ //Tercer salón
        //de 13.3m a 22.3m
        n0=n(ix,iy,iz,i);
        n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
        fnew[n0f]=D*f[n0];           
        }
      for(ix=floor(0.84*Lx)-1;ix<Lx;ix++){//Muro después del tercer salón
        for(iy=floor(0.79*Ly)-1;iy<Ly;iy++){
          n0=n(ix,iy,iz,i);
          n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
          fnew[n0f]=D*f[n0];                    
    ...
    //Para la parte superior a 2.1m de la frontera (todo pared)
    for(iz=floor(0.51*Lz)-1;iz<Lz;iz++){
      for(ix=0;ix<Lx;ix++){
        for(iy=floor(0,78*Lz)-1;iy<Ly;iy++){
            n0=n(ix,iy,iz,i);
            n0f=n(ix,iy,iz,(i-2*((int)floor((i+1)/2)-1))%2+2*((int)floor((i+1)/2)-1)+1);
            fnew[n0f]=D*f[n0];        
          }
  ...
  //Frontera trasera
  for(i=1;i<Q;i++){
    //Parte inferior a 2.5m
    for(iz=0;iz<floor(0.61*Lz);iz++){

      }
    }    
  }
  
  //Bounce back para "salones"
...
void LatticeBoltzmann::ImposeFieldsClasic(int t){
...
void LatticeBoltzmann::PrintR(const char * NameFile){
  ofstream MyFile(NameFile); double rho0; int ix,iy,iz;
  iy=Ly/2, iz=Lz/2;
  for(ix=0;ix<Lx;ix++){
    rho0=rho(ix,iy,iz,true);
...
int main(void){
  ...
  int t,tp,tsaturado=650, tmax=2500,N=6;
...
//Una vez alcanzado el equilibrio en la potencia, se apagan las fuentes y se deja evolucionar el sistema.
